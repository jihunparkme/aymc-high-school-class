...
#### 2-1. 화면 상단 정보
- [ ] 오늘 날짜 표시 (예: "2024년 2월 5일" 또는 "2024-02-05")
- [ ] 선택된 반 정보 표시 (학년, 반, 담임선생님 이름)

#### 2-2. 학생별 정보 및 버튼
- [ ] 각 학생마다 다음 요소 표시:
  - 학생 이름
  - 출석 여부 체크박스 (체크/미체크 상태 표시)
  - "기도제목" 버튼
  - "특이사항" 버튼
- [ ] 기도제목/특이사항 입력 여부 시각적 표시 (예: 버튼 색상 변경, 점 표시 등)
...
```javascript
// 오늘 날짜 데이터 (메모리와 동기화)
localStorage.setItem('schoolData_current', JSON.stringify(schoolData))
localStorage.setItem('studentData_current', JSON.stringify(studentData))

// 이전 날짜 백업
localStorage.setItem('schoolData_backup_2026-02-04', JSON.stringify(data))
localStorage.setItem('studentData_backup_2026-02-04', JSON.stringify(data))
```
